...
def printclust(clust, labels=None, n=0):
    ...
    if clust.id < 0:
        ...
        if labels == None:
            ...
            print clust.id, ' ', labels[clust.id]
    # 现在开始打印左侧分支和右侧分支
    if clust.left != None:
...
def kcluster(rows, distance=pearson, k = 4):
    ...
    # 随机创建 k 个中心点
    clusters = [
        [random.random() * (ranges[i][1] - ranges[i][0]) + ranges[i][0] for i in range(len(rows[0]))]
        for j in range(k)
    ]
